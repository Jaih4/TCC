...
def treinar_modelo():
    ...
    # num_labels=2 significa: 0 (Não Tóxico) e 1 (Tóxico)
    model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=2)

    # 2. Carregar o Dataset (Vamos usar o HateBR para este primeiro teste)
    print("Baixando dataset HateBR...")
    dataset = load_dataset("ruanchaves/hatebr", trust_remote_code=True)
    
    # O HateBR vem inteiro, então vamos dividir: 80% para treino, 20% para a prova final (teste)
    ...
    def tokenize_function(examples):
        ...
        # Força o gabarito a ser número inteiro (0 ou 1) e já cria a coluna "labels"
        tokens["labels"] = [int(label) for label in examples["offensive_language"]]
    ...
    # 4. Configurações Específicas para a RTX 4060 (8GB VRAM)
    training_args = TrainingArguments(
        ...
        weight_decay=0.01,
        fp16=True,                      # <-- O SEGREDO: Treinamento em meia-precisão (Rápido e leve)
        save_strategy="epoch",
    )
